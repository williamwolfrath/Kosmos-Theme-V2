...
//dsm($profile_node);
$college = taxonomy_get_term($profile_node->field_university_tax[0]['value']);
//dsm($profile_node->field_university_tax[0]['value']);
//dsm($college);
$user_info = ($college->name) ? ', ' . $college->name : '';
$user_info .= ($profile_node->field_academic_discipline[0]['value']) ? ', ' . $profile_node->field_academic_discipline[0]['value'] : '';
print '<h3>' . l($profile_user->name, "user/$output") . '</h3>' . $profile_user->user_type . $user_info ;
?>
</div>
<div class="clear">&nbsp;</div>
<?php //print $output; ?>